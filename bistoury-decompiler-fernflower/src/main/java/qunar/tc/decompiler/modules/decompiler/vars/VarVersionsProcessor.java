...
import qunar.tc.decompiler.modules.decompiler.exps.ConstExprent;
...
import qunar.tc.decompiler.modules.decompiler.sforms.SSAConstructorSparseEx;
...
import qunar.tc.decompiler.util.FastSparseSetFactory.FastSparseSet;
    ...
    private static void simpleMerge(VarTypeProcessor typeProcessor, DirectGraph graph, StructMethod mt) {
        ...
        for (VarVersionPair pair : mapExprentMinTypes.keySet()) {
            if (pair.version >= 0) {  // don't merge constants
                ...
                Map827.computeIfAbsent(mapVarVersions, pair.var, new Function<Integer, Set<Integer>>() {
                    @Override
