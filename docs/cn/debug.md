...
Arthas的watch命令让我们可以观察函数的入参、返回值、异常等等，然而似乎每次watch都需要去文档里看看参数如何设置，对于函数中的本地变量也是无能为力，特别是行数较大的方法，方法内部的情况还是难以明了。

这样的日子一去不复返了，我们实现了在线Debug功能，它模拟了ide的调试体验，可以直接在代码上添加断点，甚至还支持条件断点。
