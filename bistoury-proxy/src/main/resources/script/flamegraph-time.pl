...
my $titledefault = "Profiler Flame Graph";	# overwritten by --title
...
while (my ($id, $node) = each %Node) {
    ...
    # 修改代码
    # ns -> us
    my $fill_samples=sprintf "%d",$samples/1000;
    $countname="us";

    # us -> ms
    if($samples>100000000){
        $fill_samples=sprintf "%d", $samples/1000000;
        $countname="ms";
    }

    # ms -> s
    if($samples>100000000000){
        $fill_samples=sprintf "%d", $samples/1000000000;
    }

	(my $samples_txt = $fill_samples) # add commas per perlfaq5
