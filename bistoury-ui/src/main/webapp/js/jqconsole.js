...
(function() {
  ...
  JQConsole = (function() {
    ...
    JQConsole.prototype.ClearScreen = function() {
        if (this.state !== STATE_OUTPUT) {
            //this.ClearPromptText(true);
        }
        this.state = STATE_OUTPUT;
    ...
    JQConsole.prototype.SetPromptText = function(text) {
      ...
      JQConsole.prototype.AddPromptText = function(text) {
          this.$prompt_before.html('');
          this.$prompt_after.html('');
          this.$prompt_label.text(false ? '' : this._SelectPromptLabel(false));
          this.$prompt_left.text('');
          this.$prompt_right.text('');
          this._AppendPromptText(text);
          this._ScrollToEnd();
    ...
    JQConsole.prototype.UpdatePromptLabel = function() {
      ...
      JQConsole.prototype.Write = function (text, cls, escape) {
          ...
          if (index++ % length == 0) {
              className = classPrefix + clsIndex++;
              historyClass.push(className)
          }
          span = $(EMPTY_SPAN).html(text);
          if (cls != null) {
              span.addClass(cls + " " + className);
          } else {
              span.addClass(className);
          }
          if (historyClass.length > 10) {
              $("." + historyClass[0]).hide().remove();
              historyClass.shift();
          }
    ...
    JQConsole.prototype._HandleKey = function(event) {
      ...
      if (event.altKey) {
        ...
        return this._HandleCtrlShortcut(event);
...
//            break;
        ...
        switch (key) {
          case KEY_ENTER:
            this._HandleEnter(false, KEY_ENTER);
            break;
          case KEY_TAB:
            this._HandleEnter(false, KEY_TAB);
            break;
          case KEY_DELETE:
            this._Delete(false);
            break;
    ...
    JQConsole.prototype._HandleCtrlShortcut = function(event) {
    ...
    JQConsole.prototype._HandleEnter = function(shift, keyCode) {
      ...
      if (shift) {
          ...
          return function(indent) {
            ...
            if (indent !== false) {
              ...
              if (_this.history_active) {
                if (text != '' &&(!_this.history.length || _this.history[_this.history.length - 1] !== text)) {
        ...
        if (this.multiline_callback) {
          if (this.async_multiline) {
            return this.multiline_callback(text, continuation, keyCode);
          } else {
            return continuation(this.multiline_callback(text, keyCode));
          }
        } else {
